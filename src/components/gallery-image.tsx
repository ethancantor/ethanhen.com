...
export const GalleryImage = memo(function GalleryImage({ image, quality = 100, fill } : { image: string, quality?: number, fill?: boolean }){
    ...
    const { data: image_data, error: image_error, isLoading: image_loading } = useSWR(`/api/images?image=${image}&quality=${quality}`, (url) => fetch(url).then((res) => res.blob()))
    const { data: meta_data, error: meta_error, isLoading: meta_loading} = useSWR(`/api/image-data?image=${image}`, (url) => fetch(url).then((res) => res.json()))
    ...
    if(!image_error && !image_loading && image_data && meta_data && !meta_error) {
        ...
        return (
            <div className='w-full h-full'>
                ...
                <Image src={url} alt={image} width={meta_data.width || 0} height={meta_data.height || 0} className='w-fit h-fit object-cover' /> }
            </div>
        )
    }
